...
- **1-on-1 Payment (DIRECT):** Người mua trả tiền -> Nền tảng nhận toàn bộ số tiền vào tài khoản công ty.
...
- **Body:**
  ...
  ```
- **Giải thích `type`:**
  - `DIRECT`: Công ty nhận 100% số tiền, không tạo lệnh payout cho seller.
...
- **Chức năng:** PayOS tự động gọi vào đây. Hệ thống sẽ:
  ...
  3. Nếu là `COMMISSION`, đẩy lệnh chi tiền vào hàng đợi Redis (BullMQ).
...
### Cấu hình tài khoản nhận tiền cho từng luồng:

- `DIRECT`: Tiền dừng ở tài khoản merchant/công ty đã liên kết với PayOS.
- `COMMISSION`: Hệ thống lấy tài khoản ngân hàng của seller từ bảng `users` để chi 90%.
...
_(Lưu ý: dữ liệu trong bảng `users` chỉ được dùng cho luồng `COMMISSION`; `bankName` phải là mã BIN của ngân hàng theo tiêu chuẩn NAPAS/PayOS. Các biến `BANK_NAME`, `BANK_ACCOUNT`, `BANK_ACCOUNT_NAME` không còn được dùng trong luồng hiện tại.)_
