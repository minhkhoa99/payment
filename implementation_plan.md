...
1-on-1 Payment: Buyer pays Platform and the full amount stays in the Platform account.
...
Assumption on "1-on-1 Payment": Since PayOS credentials belong to the Platform, "1-on-1" means the Platform collects and keeps 100% of the payment. Commission is 10% for the intermediary flow, with the remaining 90% transferred to the Seller.
...
3. payouts (Outbound Transfers)
   ...
   amount (DECIMAL) - The amount sent to seller (90% for COMMISSION only)
   fee_amount (DECIMAL) - The platform fee (10% for COMMISSION)
...
1. Payment Controller (Inbound HTTP)
   ...
   Body: PayOS JSON
   Action: Verify signature -> Update payments.status -> Trigger Payout Job only for COMMISSION payments.
...
2. Payout Job (Background Worker - BullMQ/Redis)
   Trigger: When a COMMISSION payment status becomes PAID.
   Logic:
   If type == DIRECT: No payout is created; funds remain in the Platform account.
